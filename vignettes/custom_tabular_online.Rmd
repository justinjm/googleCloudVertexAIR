...
```{r}
sh('gcloud artifacts repositories create {private_repo} --repository-format=docker --location={gcva_project_get()} --description="Docker repository"')
```
...
* API [REST Resource: projects.locations.customJobs  |  Vertex AI  |  Google Cloud](https://cloud.google.com/vertex-ai/docs/reference/rest/v1/projects.locations.customJobs)
